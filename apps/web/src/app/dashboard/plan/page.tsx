...
export default async function PlanPage() {
  ...
  // #region agent log
  fetch("http://127.0.0.1:7257/ingest/a9e25fe2-a3a6-41a5-b2f2-fc188fac1d73", { method: "POST", headers: { "Content-Type": "application/json", "X-Debug-Session-Id": "126be4" }, body: JSON.stringify({ sessionId: "126be4", runId: "allocator-output-debug", hypothesisId: "H1", location: "apps/web/src/app/dashboard/plan/page.tsx:allocationSlices", message: "allocation slices summary", data: { weekSlices: ctx.weekSlices.length, perSlice: allocationSlices.map((a, i) => ({ i, blocks: a.blocks.length, scheduledMinutes: a.metrics.utilisation.scheduledMinutes, weekCapacityMinutes: a.metrics.utilisation.weekCapacityMinutes, busySlice: ctx.weekSlices[i]?.busy.length ?? 0, daySheetGoalBusySlice: ctx.weekSlices[i]?.daySheetGoalBusy.length ?? 0, systemBlocksSlice: ctx.weekSlices[i]?.systemBlocks.length ?? 0 })) }, timestamp: Date.now() }) }).catch(() => {});
  // #endregion
