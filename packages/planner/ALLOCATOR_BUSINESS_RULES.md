...
- **Pass 2:** Splits the **remainder** of `weekCapacityMinutes` across eligible goals (`allocationSharePercent` weighting + equal share for others). Under **hybrid** or global **stacked** goal-window mode, **stacked-role** rows that set **`allocationSharePercent`** skip this remainder cohort: they receive **`plannedWeeklyMinutes ≈` quantised `% × weekCapacityMinutes`** afterward (same ceiling as linear `%`, respecting **`maxMinutesPerWeek`**), so `%` drives stacked timemap sizing without shrinking how much remainder **linear** peers (and stacked peers **without** `%`) split. Packing mode (`even` vs `finish-early`) affects **calendar layout only**, not these minute totals.
...
- Pass 1+2 are unchanged except **`allocationSharePercent` on stacked-role rows** under hybrid/stacked window mode (see Pass‑2 bullet above): `%` sets stacked targets from **`% × weekCapacityMinutes`** without consuming Pass‑2 remainder.
